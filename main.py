...
def servo_Angle(angle, idServo):
    ...
    if idServo == 'd':
        disparador.goto(round(Map(angle,0,180,0,1024)))
...
while True:
    ...
    if uart.any():
        ...
        if modo == True and command == b'q':
            dirServo = 'q'
        #MOVIMIENTO DISPARADOR
        #prendido
        if modo == True and command == b'm':
            dirServo = 'm'
        if modo == True and command == b'r':
            dirServo = 'a'
        #Abajo
        if modo == True and command == b'n':
            dirServo = 'n'
    ...
    #Disparador
    if dirServo == 'm':
        if posSerD < -10:#Holgura en caso de que oprima el boton de mas
            posSerD =0
        elif posSerD > 180:
            posSerD =180
        else:
            posSerD = posSerD - angulosD
        servo_Angle(posSerD, 'd')
        utime.sleep(0.001)
    if dirServo == 'a':
        if posSerD < 0:
